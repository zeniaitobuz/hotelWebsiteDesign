...
import * as links from "./js/links.mjs";
  ...
  try {
    ...
    req.on("end", () => {
      ...
      if (fullFormdata.length > 0) {
        (async function () {
          const dataFromFile = await fs.readFile("./js/formData.txt", "utf8");
          fullFormdata += ";" + dataFromFile;
          await fs.writeFile("./js/formData.txt", fullFormdata);
        })();
      }
    });
